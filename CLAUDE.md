...
graph TD
    ...
    style RHA fill:#238636,color:#fff
...
│   │   ├── scan-pipelines.md
│   │   ├── scan-prs.md
│   │   ├── scan-repo-health.md      # Health scanner across 6 categories
│   │   ├── repo-sync.md
│   │   ├── repo-status.md
│   │   ├── maintain.md
│   │   ├── distribute-workflows.md
│   │   ├── swarm.md                  # Main swarm orchestrator
│   │   └── unleash.md
│   ├── skills/
│   │   ├── issue-triage/SKILL.md
│   │   ├── pr-review/SKILL.md
│   │   ├── pipeline-fix/SKILL.md
│   │   ├── repo-sync/SKILL.md
│   │   ├── repo-config.md
│   │   ├── readme-docs/SKILL.md      # README & documentation freshness
│   │   ├── license-compliance/SKILL.md # License compliance checks
│   │   ├── workflow-maintenance/SKILL.md # GitHub Actions maintenance
│   │   ├── docs-health/SKILL.md      # Community health files
│   │   ├── git-hygiene/SKILL.md      # .gitignore, stale branches, large files
│   │   └── cicd-maintenance/SKILL.md # Proactive CI/CD maintenance
│   └── .swarm-state.json             # Swarm state persistence
└── .github/
    └── workflows/
...
| `/scan-repo-health [repo\|all]` | Scan repos for health issues across 6 categories (README, license, workflows, community files, git hygiene, CI/CD) |
...
| `/swarm` | Launch full parallel agent swarm — 4 discovery agents + N worker agents |
...
| `/readme-docs [repo]` | Check and fix README quality: badges, links, required sections, code examples |
| `/license-compliance [repo]` | Validate LICENSE file, match against package metadata, check dependency conflicts |
| `/workflow-maintenance [repo]` | Audit GitHub Actions: outdated action versions, deprecated refs, missing best practices |
| `/docs-health [repo]` | Add missing community files: CONTRIBUTING, CODE_OF_CONDUCT, SECURITY, CODEOWNERS, templates |
| `/git-hygiene [repo]` | Fix .gitignore, flag large tracked files, delete stale merged branches |
| `/cicd-maintenance [repo]` | Clean old artifacts, update deprecated runners, identify chronic failure patterns |
...
graph LR
    ...
    M --> SI[/scan-issues]
    M --> SPR[/scan-prs]
    M --> SRH[/scan-repo-health]
    SP -->|failures found| PF[/pipeline-fix skill]
    SI -->|issues found| IT[/issue-triage skill]
    SPR -->|PRs found| PR[/pr-review skill]
    SRH -->|README issues| RD[/readme-docs skill]
    SRH -->|license issues| LC[/license-compliance skill]
    SRH -->|workflow issues| WM[/workflow-maintenance skill]
    SRH -->|missing files| DH[/docs-health skill]
    SRH -->|git issues| GH2[/git-hygiene skill]
    SRH -->|CI/CD issues| CM[/cicd-maintenance skill]
...
graph TD
    ...
    S --> D["Phase 1: Discovery<br/>(4 parallel Task agents)"]
    ...
    D --> RHS["Repo Health Scanner<br/>README, license, workflows,<br/>community files, git, CI/CD"]
    PS --> T["Phase 2: Triage<br/>6-level priority queue"]
    IS --> T
    PRS --> T
    RHS --> T
    ...
    E --> HEALTH_HI["Fix high-severity<br/>repo health"]
    E --> TRIAGE["Triage issues"]
    E --> REVIEW["Review PRs"]
    E --> HEALTH_LO["Fix medium/low<br/>repo health"]
...
**Safety rails**: Max 10 fix commits per iteration. Max 15 repo health fixes per iteration. Max 20 ralph-loop iterations. Rate limit check at start of each iteration (stops if < 500 remaining). Never auto-merges non-bot PRs. Never modifies dispatch.yaml. Never deletes protected branches. Never overwrites existing community files or LICENSE.
